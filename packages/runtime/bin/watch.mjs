/**
 * Shared watch implementation for `inception watch` and `inception-watch`:
 * tail a run's events.ndjson and render live progress.
 *
...
export function watchCommand(args, usagePrefix = 'inception-watch') {
  const runDir = args.find(a => !a.startsWith('--'));
  const replay = args.includes('--replay');
  if (!runDir) {
    console.error(`usage: ${usagePrefix} <runDir> [--replay]`);
    process.exit(1);
  }
